...
std::vector<TH2D*> h_FRONT_LENGTH_XY_DUMMY;
//xy distribution for primaries scattered off from in front of the magnetic field

std::vector<TH2D*> h_FRONT_LENGTH_NAIVE_XY_DUMMY;
//xy distribution for naive primaries scattered off from in front of the magnetic field

std::vector<TH2D*> h_BACK_LENGTH_XY_DUMMY;
//xy distribution for primaries scattered off from behind the magnetic field

std::vector<TH2D*> h_BACK_LENGTH_NAIVE_XY_DUMMY;
//xy distribution for naive primaries scattered off from behind the magnetic field

std::vector<TH2D*> h_VERY_FRONT_LENGTH_XY_DUMMY;
//xy distribution for primaries scattered off from behind the cell

std::vector<TH2D*> h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY;
//xy distribution for naive primaries scattered off from behind the cell

std::vector<TH2D*> h_VERY_BACK_LENGTH_XY_DUMMY;
//xy distribution for primaries scattered off from in front of the cell

std::vector<TH2D*> h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY;
//xy distribution for naive primaries scattered off from in front of the cell
...
void Z_Cut_Distribution(std::string filename, std::string DetectorType)
{
...
void GetGitInfo()
{
  ...
  /*
   //debugging
   // As an example, we'll print out th+e entire runInfo, so you can see what we have
   // extracted.
...
void Define_Histograms_Dummy()
{
  ...
  h_FRONT_LENGTH_XY_DUMMY.resize(NUM_PRIMARIES);
  h_FRONT_LENGTH_NAIVE_XY_DUMMY.resize(NUM_PRIMARIES);
  h_BACK_LENGTH_XY_DUMMY.resize(NUM_PRIMARIES);
  h_BACK_LENGTH_NAIVE_XY_DUMMY.resize(NUM_PRIMARIES);
  h_VERY_FRONT_LENGTH_XY_DUMMY.resize(NUM_PRIMARIES);
  h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY.resize(NUM_PRIMARIES);
  h_VERY_BACK_LENGTH_XY_DUMMY.resize(NUM_PRIMARIES);
  h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY.resize(NUM_PRIMARIES);
  ...
  for (int i = 0; i < NUM_PRIMARIES; i++)
  {
    ...
    //-22.5 to -10
    //all primaries
    h_BACK_LENGTH_XY_DUMMY[i] = new TH2D(
        Form("h_BACK_LENGTH_XY_DUMMY[%d]", i),
        Form("XY distribution for %s generated behind the gas target",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //naive primaries
    h_BACK_LENGTH_NAIVE_XY_DUMMY[i] = new TH2D(
        Form("h_BACK_LENGTH_NAIVE_XY_DUMMY[%d]", i),
        Form("XY distribution for naive %s generated behind the gas target",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //10 to 22.5
    //all primaries
    h_FRONT_LENGTH_XY_DUMMY[i] = new TH2D(
        Form("h_FRONT_LENGTH_XY_DUMMY[%d]", i),
        Form("XY distribution for %s generated in front of the gas target",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //naive primaries
    h_FRONT_LENGTH_NAIVE_XY_DUMMY[i] = new TH2D(
        Form("h_FRONT_LENGTH_NAIVE_XY_DUMMY[%d]", i),
        Form("XY distribution for %s generated in front of the gas target",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);
    ...
    //-22.5 to -20
    //all primaries
    h_VERY_BACK_LENGTH_XY_DUMMY[i] = new TH2D(
        Form("h_VERY_BACK_LENGTH_XY_DUMMY[%d]", i),
        Form("XY distribution for %s generated behind the cell",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //naive primaries
    h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[i] = new TH2D(
        Form("h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[%d]", i),
        Form("XY distribution for naive %s generated behind the cell",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //20 to 22.5
    //all primaries
    h_VERY_FRONT_LENGTH_XY_DUMMY[i] = new TH2D(
        Form("h_VERY_FRONT_LENGTH_XY_DUMMY[%d]", i),
        Form("XY distribution for %s generated in front of the cell",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);

    //naive primaries
    h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[i] = new TH2D(
        Form("h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[%d]", i),
        Form("XY distribution for naive %s generated in front of the cell",
             INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()),
        680, -.5, .5, 680, -.5, .5);
      ...
      if (Dummy_Det->fHits.size() != 0)
      {
        ...
        for (int i_hits = 0; i_hits < detector_hits_nentries; i_hits++)
        {
          ...
          if (Dummy_Det->fHits[i_hits].fParentID == 0)
          {
            ...
            if ((z > -.225) and (z < -0.2))
            {
              h_VERY_BACK_LENGTH_XY_DUMMY[0]->Fill(x, y);
            }
            if ((z > -.225) and (z < -0.1))
            {
              h_BACK_LENGTH_XY_DUMMY[0]->Fill(x, y);
            }
            if ((z < .225) and (z > 0.2))
            {
              h_VERY_FRONT_LENGTH_XY_DUMMY[0]->Fill(x, y);
            }
            if ((z < .225) and (z > 0.1))
            {
              h_FRONT_LENGTH_XY_DUMMY[0]->Fill(x, y);
            }
            ...
            if (Dummy_Det->fHits[i_hits].fPrimaryID == 1)
            {
              ...
              if ((z > -.225) and (z < -0.2))
              {
                h_VERY_BACK_LENGTH_XY_DUMMY[1]->Fill(x, y);
              }
              if ((z > -.225) and (z < -0.1))
              {
                h_BACK_LENGTH_XY_DUMMY[1]->Fill(x, y);
              }
              if ((z < .225) and (z > 0.2))
              {
                h_VERY_FRONT_LENGTH_XY_DUMMY[1]->Fill(x, y);
              }
              if ((z < .225) and (z > 0.1))
              {
                h_FRONT_LENGTH_XY_DUMMY[1]->Fill(x, y);
              }
              /*
               //debug
            ...
            if (Dummy_Det->fHits[i_hits].fPrimaryID == 2)
            {
              ...
              if ((z > -.225) and (z < -0.2))
              {
                h_VERY_BACK_LENGTH_XY_DUMMY[2]->Fill(x, y);
              }
              if ((z > -.225) and (z < -.1))
              {
                h_BACK_LENGTH_XY_DUMMY[2]->Fill(x, y);
              }
              if ((z < .225) and (z > .2))
              {
                h_VERY_FRONT_LENGTH_XY_DUMMY[2]->Fill(x, y);
              }
              if ((z < .225) and (z > .1))
              {
                h_FRONT_LENGTH_XY_DUMMY[2]->Fill(x, y);
              }
              //debugging
...
//                  <<std::endl;
    ...
    if (Primary_1_Hit_Num != -1000)
    {
      ...
      if ((z > -.225) and (z < -.2))
      {
        h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[1]->Fill(x, y);
      }
      if ((z > -.225) and (z < -.1))
      {
        h_BACK_LENGTH_NAIVE_XY_DUMMY[1]->Fill(x, y);
      }
      if ((z < .225) and (z > .2))
      {
        h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[1]->Fill(x, y);
      }
      if ((z < .225) and (z > .1))
      {
        h_FRONT_LENGTH_NAIVE_XY_DUMMY[1]->Fill(x, y);
      }
    ...
    if (Primary_2_Hit_Num != -1000)
    {
      ...
      if ((z > -.225) and (z < -.2))
      {
        h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[2]->Fill(x, y);
      }
      if ((z > -.225) and (z < -.1))
      {
        h_BACK_LENGTH_NAIVE_XY_DUMMY[2]->Fill(x, y);
      }
      if ((z < .225) and (z > .2))
      {
        h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[2]->Fill(x, y);
      }
      if ((z < .225) and (z > .1))
      {
        h_FRONT_LENGTH_NAIVE_XY_DUMMY[2]->Fill(x, y);
      }
    }
    ...
    if (Primary_1_Hit_Num != -1000 && Primary_2_Hit_Num != -1000)
    {
      ...
      if ((z > -.225) and (z < -.2))
      {
        h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_1, y_1);
      }
      if ((z > -.225) and (z < -.1))
      {
        h_BACK_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_1, y_1);
      }
      if ((z < .225) and (z > .2))
      {
        h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_1, y_1);
      }
      if ((z < .225) and (z > .1))
      {
        h_FRONT_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_1, y_1);
      }
      ...
      if ((z > -.225) and (z < -.2))
      {
        h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_2, y_2);
      }
      if ((z > -.225) and (z < -.1))
      {
        h_BACK_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_2, y_2);
      }
      if ((z < .225) and (z > .2))
      {
        h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_2, y_2);
      }
      if ((z < .225) and (z > .1))
      {
        h_FRONT_LENGTH_NAIVE_XY_DUMMY[0]->Fill(x_2, y_2);
      }
    }
...
void Plot_Dummy()
{
  for (int i = 0; i < NUM_PRIMARIES; i++)
  {
    ...
    //all primaries
    TCanvas* c_Very_Back_Length_Dummy = new TCanvas(
        "c_Very_Back_Length_Dummy",
        Form(
            "XY hit distribution for primary %d generated from behind the cell",
            i),
        650, 650);

    h_VERY_BACK_LENGTH_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis* p_Very_Back_Length_XY_Dummy =
        (TPaletteAxis*) h_VERY_BACK_LENGTH_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
    //debugging
    std::cout << "palette = " << p_Very_Back_Length_XY_Dummy << std::endl;
//    //moves z axis to right side and width (X2 -X1)
//    p_Very_Back_Length_XY_Dummy->SetX1NDC(0.9);
//    p_Very_Back_Length_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_VERY_BACK_LENGTH_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    c_Very_Back_Length_Dummy->Update();

    c_Very_Back_Length_Dummy->SaveAs(
        Form("%s/Behind_Cell_XY_Distribution_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Very_Back_Length_Dummy->SaveAs(
        Form("%s/Behind_Cell_Target_XY_Distribution_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //naive primaries
    TCanvas* c_Very_Back_Length_Naive_Dummy =
        new TCanvas(
            "c_Very_Back_Length_Naive_Dummy",
            Form(
                "XY hit distribution for naive primary %d generated from behind the cell",
                i),
            650, 650);

    h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Very_Back_Length_Naive_XY_Dummy =
        (TPaletteAxis*) h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Very_Back_Length_Naive_XY_Dummy->SetX1NDC(0.9);
//    p_Very_Back_Length_Naive_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_VERY_BACK_LENGTH_NAIVE_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Very_Back_Length_Naive_Dummy->Update();

    c_Very_Back_Length_Naive_Dummy->SaveAs(
        Form("%s/Behind_Cell_Target_XY_Distribution_naive_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Very_Back_Length_Naive_Dummy->SaveAs(
        Form("%s/Behind_Cell_Target_XY_Distribution_naive_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //back length

    //all primaries
    TCanvas* c_Back_Length_Dummy =
        new TCanvas(
            "c_Back_Length_Dummy",
            Form(
                "XY hit distribution for primary %d generated from behind the effective target",
                i),
            650, 650);

    h_BACK_LENGTH_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Back_Length_XY_Dummy =
        (TPaletteAxis*) h_BACK_LENGTH_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Back_Length_XY_Dummy->SetX1NDC(0.9);
//    p_Back_Length_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_BACK_LENGTH_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Back_Length_Dummy->Update();

    c_Back_Length_Dummy->SaveAs(
        Form("%s/Behind_Gas_XY_Distribution_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Back_Length_Dummy->SaveAs(
        Form("%s/Behind_Gas_XY_Distribution_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //naive primaries
    TCanvas* c_Back_Length_Naive_Dummy =
        new TCanvas(
            "c_Back_Length_Naive_Dummy",
            Form(
                "XY hit distribution for naive primary %d generated from behind the effective target",
                i),
            650, 650);

    h_BACK_LENGTH_NAIVE_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Back_Length_Naive_XY_Dummy =
        (TPaletteAxis*) h_BACK_LENGTH_NAIVE_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Back_Length_Naive_XY_Dummy->SetX1NDC(0.9);
//    p_Back_Length_Naive_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_BACK_LENGTH_NAIVE_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Back_Length_Naive_Dummy->Update();

    c_Back_Length_Naive_Dummy->SaveAs(
        Form("%s/Behind_Gas_XY_Distribution_naive_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Back_Length_Naive_Dummy->SaveAs(
        Form("%s/Behind_Gas_XY_Distribution_naive_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //front length

    //all primaries
    TCanvas* c_Front_Length_Dummy =
        new TCanvas(
            "c_Front_Length_Dummy",
            Form(
                "XY hit distribution for primary %d generated from in front of the effective target",
                i),
            650, 650);

    h_FRONT_LENGTH_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Front_Length_XY_Dummy =
        (TPaletteAxis*) h_FRONT_LENGTH_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Front_Length_XY_Dummy->SetX1NDC(0.9);
//    p_Front_Length_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_FRONT_LENGTH_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Front_Length_Dummy->Update();

    c_Front_Length_Dummy->SaveAs(
        Form("%s/Front_of_Gas_XY_Distribution_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Front_Length_Dummy->SaveAs(
        Form("%s/Front_of_Gas_XY_Distribution_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //naive primaries
    TCanvas* c_Front_Length_Naive_Dummy =
        new TCanvas(
            "c_Front_Length_Naive_Dummy",
            Form(
                "XY hit distribution for naive primary %d generated from in front of the effective target",
                i),
            650, 650);

    h_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Front_Length_Naive_XY_Dummy =
        (TPaletteAxis*) h_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");

//    //moves z axis to right side and width (X2 -X1)
//    p_Front_Length_Naive_XY_Dummy->SetX1NDC(0.9);
//    p_Front_Length_Naive_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Front_Length_Naive_Dummy->Update();

    c_Front_Length_Naive_Dummy->SaveAs(
        Form("%s/Front_of_Gas_XY_Distribution_naive_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Front_Length_Naive_Dummy->SaveAs(
        Form("%s/Front_of_Gas_XY_Distribution_naive_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //very front length

    //all primaries
    TCanvas* c_Very_Front_Length_Dummy =
        new TCanvas(
            "c_Very_Front_Length_Dummy",
            Form(
                "XY hit distribution for primary %d generated from in front of the cell",
                i),
            650, 650);

    h_VERY_FRONT_LENGTH_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Very_Front_Length_XY_Dummy =
        (TPaletteAxis*) h_VERY_FRONT_LENGTH_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Very_Front_Length_XY_Dummy->SetX1NDC(0.9);
//    p_Very_Front_Length_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_VERY_FRONT_LENGTH_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Very_Front_Length_Dummy->Update();

    c_Very_Front_Length_Dummy->SaveAs(
        Form("%s/Front_of_Cell_XY_Distribution_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Very_Front_Length_Dummy->SaveAs(
        Form("%s/Front_of_Cell_Target_XY_Distribution_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));

    //naive primaries
    TCanvas* c_Very_Front_Length_Naive_Dummy =
        new TCanvas(
            "c_Very_Front_Length_Naive_Dummy",
            Form(
                "XY hit distribution for naive primary %d generated from in front of the cell",
                i),
            650, 650);

    h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->Draw("colz");
    ...
    //reformats colored z axis
    TPaletteAxis *p_Very_Front_Length_Naive_XY_Dummy =
        (TPaletteAxis*) h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
//    //moves z axis to right side and width (X2 -X1)
//    p_Very_Front_Length_Naive_XY_Dummy->SetX1NDC(0.9);
//    p_Very_Front_Length_Naive_XY_Dummy->SetX2NDC(0.91);
    //set z axis tick length
    h_VERY_FRONT_LENGTH_NAIVE_XY_DUMMY[i]->GetZaxis()->SetTickLength(.01);

    gStyle->SetStatY(.9);
    gStyle->SetStatX(.9);

    c_Very_Front_Length_Naive_Dummy->Update();

    c_Very_Front_Length_Naive_Dummy->SaveAs(
        Form("%s/Front_of_Cell_Target_XY_Distribution_naive_primary_%d.C",
             OUTPUT_PREFIX.c_str(), i));
    c_Very_Front_Length_Naive_Dummy->SaveAs(
        Form("%s/Front_of_Cell_Target_XY_Distribution_naive_primary_%d.png",
             OUTPUT_PREFIX.c_str(), i));
  }
}
