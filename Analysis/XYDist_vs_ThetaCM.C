/***********************************************************
 ...
 Purpose:
 - To take a root file and plots the niave primary hits on the detector plane vs
 the Center of Mass theta angle
 - It will work for BOTH dummy and left right detectors :) When the LeftRight is
 set up.

 - We are using TProfile2D so that it will be able to have the theta information on
 the Z-axis. Through use of the following
 - new TProfile2D("histo" title, xbins, xmin, xmax, ybins, ymin, ymax, zmin, zmax)
 - Fill(x, y, z)
 - Draw("colz")
...
//define histograms
//DUMMY DET
std::vector<TProfile2D*> h_XY_DIST_VS_THETA_CM_DUMMY;  //[primary e]
//XY distribution vs theta CM for naive primary hits for each primary Dummy Det

//LEFT RIGHT DET
std::vector<TProfile2D*> h_XY_DIST_VS_THETA_CM_LEFTRIGHT;  //[primary e]
//XY distribution vs theta CM for naive primary hits for each primary left Right Det
...
static std::string OUTPUT_PREFIX =
    "/home/vmgray/workspace/HMolPol/Simulation_Analysis/commit_5a398bd_Thesis/11GeV_Mag8T_NoOptics/XY_Dist_vs_ThetaCM";
...
void XYDist_vs_ThetaCM(std::string filename, std::string DetectorType)
{
...
//  HMOLPOL_CHAIN->Add(
//      "/home/vmgray/workspace/HMolPol/Simulation_Analysis/commit_5a398bd_Thesis/BeamE_Scan/2_2GeV/Rootfiles/HMolPol_Dummy_Kryptonite_2_2GeV_Mag8T_-10GenZ10_0Phi360_75Theta105_NoRaster_NoOptics_5a398bd_*.root");

   HMOLPOL_CHAIN->Add(
   "~/workspace/HMolPol/Simulation_Analysis/commit_5a398bd_Thesis/11GeV_Mag8T_NoOptics/Rootfiles/*.root");
...
{
  //resize histograms
  h_XY_DIST_VS_THETA_CM_DUMMY.resize(NUM_PRIMARIES);

  //finish defining histograms
  for (int i = 0; i < NUM_PRIMARIES; i++)
  {
    // For the MeV plots - remeber x then y
    // 110, -1.05, 1.05, 110, -1.05, 1.05, 75, 105)

    // For the GeV plots
    // 110, -0.25, 0.25, 110, -0.25, 0.25, 75, 105)

    // For the 11 GeV plots No Solenoid & No Dipole
    // 300, -0.15, 0.15, 300, -0.03, 0.03, 75, 105)

    h_XY_DIST_VS_THETA_CM_DUMMY[i] = new TProfile2D(
        Form("h_XY_DIST_VS_THETA_CM_DUMMY[%d]", i),
        Form("XY vs. #theta_{CM} for naive %s on the Dummy detector",
            INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()), 300, -0.12, 0.12, 300,
        -0.03, 0.03, 75, 105);

    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetXaxis()->SetTitle(
        Form("X hit position [%s]", LENGTH_UNIT_NAME.c_str()));

    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetYaxis()->SetTitle(
        Form("Y hit position [%s]", LENGTH_UNIT_NAME.c_str()));

//    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetZaxis()->SetTitle(
//        Form("#theta_{CM} [%s]", ANGLE_UNIT_NAME.c_str()));
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetZaxis()->SetTitle("#theta_{CM} [#circ]");
  }
...
void Loop_Through_Tree_Dummy()
{
  ...
  for (int i_entry = 0; i_entry < nentries; i_entry++)
  {
    ...
    if (Primary_1_Hit_Num != -1000)
    {
      ...
      h_XY_DIST_VS_THETA_CM_DUMMY[1]->Fill(
          Dummy_Det->fHits[Primary_1_Hit_Num].fPosition.X() / LENGTH_UNIT,
          Dummy_Det->fHits[Primary_1_Hit_Num].fPosition.Y() / LENGTH_UNIT,
    ...
    if (Primary_2_Hit_Num != -1000)
    {
      ...
      h_XY_DIST_VS_THETA_CM_DUMMY[2]->Fill(
          Dummy_Det->fHits[Primary_2_Hit_Num].fPosition.X() / LENGTH_UNIT,
          Dummy_Det->fHits[Primary_2_Hit_Num].fPosition.Y() / LENGTH_UNIT,
    ...
    if (Primary_1_Hit_Num != -1000 && Primary_2_Hit_Num != -1000)
    {
      ...
      h_XY_DIST_VS_THETA_CM_DUMMY[0]->Fill(
          Dummy_Det->fHits[Primary_1_Hit_Num].fPosition.X() / LENGTH_UNIT,
          Dummy_Det->fHits[Primary_1_Hit_Num].fPosition.Y() / LENGTH_UNIT,
          Primary->fThetaCenterOfMass1 / ANGLE_UNIT);
...
void Plot_Dummy()
{
  ...
  for (int i = 0; i < NUM_PRIMARIES; i++)
  {
    //create canvas for XYDist_vs_ThetaCM
    TCanvas* c_xy_dist_vs_thetaCM_dummy = new TCanvas(
        "c_xy_dist_vs_thetaCM_dummy",
        Form("XY vs. #theta_{CM} for naive %s on the Dummy detector",
            INDEX_TO_NUM_PRIMARIES_TO_NAME[i].c_str()), 650, 650);

    //set the Z-axis range
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetZaxis()->SetRangeUser(70, 110);
//    h_XY_DIST_VS_THETA_CM_DUMMY[i]->SetBins(400, -1.5, 1.5, 400,
//        -1.5, 1.5, 80, 70, 110);

    //draw the histogram
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->Draw("colz");
//    h_XY_DIST_VS_THETA_CM_DUMMY[i]->Draw();

    gPad->Update();

    //offset the Y axis titles
    //For MeV plots
    //h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetYaxis()->SetTitleOffset(1.2);
    //For GeV Plots
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetYaxis()->SetTitleOffset(1.4);

    //Modifiy the Z axis
    gPad->SetRightMargin(0.12);  //15 per cent of pad for right margin
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetZaxis()->SetTitleOffset(1.4);

    //reformats colored z axis
    TPaletteAxis* palette_xy_dist_vs_thetaCM_dummy =
        (TPaletteAxis*) h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetListOfFunctions()->FindObject(
            "palette");
    //moves z axis to right side and width (X2 -X1)
    palette_xy_dist_vs_thetaCM_dummy->SetX1NDC(0.88);
    palette_xy_dist_vs_thetaCM_dummy->SetX2NDC(0.90);
    //set z axis tick length
    h_XY_DIST_VS_THETA_CM_DUMMY[i]->GetZaxis()->SetTickLength(0.01);
...
//   gStyle->SetOptStat("emr");  //1110);
    ...
    c_xy_dist_vs_thetaCM_dummy->Update();

    //Save the plots
    c_xy_dist_vs_thetaCM_dummy->SaveAs(
        Form("%s/XY_Dist_vs_ThetaCM_primary_%d.C", OUTPUT_PREFIX.c_str(), i));
    c_xy_dist_vs_thetaCM_dummy->SaveAs(
        Form("%s/XY_Dist_vs_ThetaCM_primary_%d.png", OUTPUT_PREFIX.c_str(), i));
  }
