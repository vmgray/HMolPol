...
//Dummy detector
//
std::vector<vector<TH2D*>> h_FRONT_FIELD_XY_DIST_DUMMY;
//1 cm cuts from 0 to 20 cm in the generation area

std::vector<vector<TH2D*>> h_BACK_FIELD_XY_DIST_DUMMY;
//1 cm cuts from -20 to 0 cm in the generation area
...
static std::string INPUT_PREFIX =
    "/home/csimpson/workspace/HMolPol/data_Simulation_Analysis/commit_19095aa/PrimaryGenZ_45cm/Dummy_G4_LEAD_GLASS/200000_Events";

//string for the where the output will go.
/*static std::string OUTPUT_PREFIX =
 "/home/vmgray/workspace/HMolPol/Simulation_Analysis/commit_06d91f8";*/
static std::string OUTPUT_PREFIX =
    "/home/csimpson/workspace/HMolPol/Simulation_Analysis/commit_19095aa/PrimaryGenZ_45cm/Dummy_G4_LEAD_GLASS/200000_Events";

std::string SAVE_PREFIX[NUM_PRIMARIES] =
    { "/home/csimpson/workspace/HMolPol/Simulation_Analysis/commit_19095aa/PrimaryGenZ_45cm/Dummy_G4_LEAD_GLASS/200000_Events/1_mm_Z_Cut_XY_Dist_Primary_0",
      "/home/csimpson/workspace/HMolPol/Simulation_Analysis/commit_19095aa/PrimaryGenZ_45cm/Dummy_G4_LEAD_GLASS/200000_Events/1_mm_Z_Cut_XY_Dist_Primary_1",
      "/home/csimpson/workspace/HMolPol/Simulation_Analysis/commit_19095aa/PrimaryGenZ_45cm/Dummy_G4_LEAD_GLASS/200000_Events/1_mm_Z_Cut_XY_Dist_Primary_2" };
...
void Define_Histograms_Dummy()
{
  ...
  h_FRONT_FIELD_XY_DIST_DUMMY.resize(NUM_PRIMARIES);
  h_BACK_FIELD_XY_DIST_DUMMY.resize(NUM_PRIMARIES);

  for (int i = 0; i < NUM_PRIMARIES; i++)
  {
    h_FRONT_FIELD_XY_DIST_DUMMY[i].resize(NUM_CUTS);
    h_BACK_FIELD_XY_DIST_DUMMY[i].resize(NUM_CUTS);
  }
  ...
  for (int primary_ID = 0; primary_ID < NUM_PRIMARIES; primary_ID++)
  {
    ...
    {
      h_FRONT_FIELD_XY_DIST_DUMMY[primary_ID][cut_position] = new TH2D(
          Form("h_FRONT_FIELD_XY_DIST_DUMMY[%d][%d]", primary_ID, cut_position),
          Form("XY distribution for %s generated from %d to %d %s",
               INDEX_TO_NUM_PRIMARIES_TO_NAME[primary_ID].c_str(), cut_position,
               cut_position + 1, Z_POSITION_UNIT_NAME.c_str()),
          600, -.5, .5, 600, -.5, .5);
      h_FRONT_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->GetXaxis()->SetTitle(
          Form("X Position (%s)", POSITION_UNIT_NAME.c_str()));
      h_FRONT_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->GetYaxis()->SetTitle(
          Form("Y Position (%s)", POSITION_UNIT_NAME.c_str()));

      h_BACK_FIELD_XY_DIST_DUMMY[primary_ID][cut_position] = new TH2D(
          Form("h_BACK_FIELD_XY_DIST_DUMMY[%d][%d]", primary_ID, cut_position),
          Form("XY distribution for %s generated from %d to %d %s",
               INDEX_TO_NUM_PRIMARIES_TO_NAME[primary_ID].c_str(),
               cut_position*-1 - 1, cut_position*-1, Z_POSITION_UNIT_NAME.c_str()),
          600, -.5, .5, 600, -.5, .5);
      h_BACK_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->GetXaxis()->SetTitle(
          Form("X Position (%s)", POSITION_UNIT_NAME.c_str()));
      h_BACK_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->GetYaxis()->SetTitle(
          Form("Y Position (%s)", POSITION_UNIT_NAME.c_str()));
    }
  }
}
        ...
        for (int i_hits = 0; i_hits < detector_hits_nentries; i_hits++)
        {
          ...
          if (Dummy_Det->fHits[i_hits].fParentID == 0)
          {
            ...
            for (int cut_position = 0; cut_position < NUM_CUTS; cut_position++)
            {
              ...
              {
                h_FRONT_FIELD_XY_DIST_DUMMY[0][cut_position]->Fill(x, y);
              }
              if (z < cut_position * -1 and z > cut_position * -1 - 1)
              {
                h_BACK_FIELD_XY_DIST_DUMMY[0][cut_position]->Fill(x, y);
              }
            }
            ...
            if (Dummy_Det->fHits[i_hits].fPrimaryID == 1)
            {
              ...
              for (int cut_position = 0; cut_position < NUM_CUTS;
                  cut_position++)
              {
                if (z > cut_position and z < cut_position + 1)
                {
                  h_FRONT_FIELD_XY_DIST_DUMMY[1][cut_position]->Fill(x, y);
                }
                if (z < cut_position * -1 and z > cut_position * -1 - 1)
                {
                  h_BACK_FIELD_XY_DIST_DUMMY[1][cut_position]->Fill(x, y);
                }
              }
              /*
               //debug
            ...
            if (Dummy_Det->fHits[i_hits].fPrimaryID == 2)
            {
                  ...
                  cut_position++)
              {
                if (z > cut_position and z < cut_position + 1)
                {
                  h_FRONT_FIELD_XY_DIST_DUMMY[2][cut_position]->Fill(x, y);
                }
                if (z < cut_position * -1 and z > cut_position * -1 - 1)
                {
                  h_BACK_FIELD_XY_DIST_DUMMY[2][cut_position]->Fill(x, y);
                }
              }
              //debugging
...
void Plot_Dummy()
{
  for (int primary_ID = 0; primary_ID < NUM_PRIMARIES; primary_ID++)
  {
    for (int cut_position = 0; cut_position < NUM_CUTS; cut_position++)
    {
      ...
      TCanvas* c_Front_Field_XY_Dist_Dummy = new TCanvas(
          "c_Front_Field_XY_Dist_Dummy",
          Form("XY hit distribution for primary %d generated from %d to %d",
               primary_ID, cut_position, cut_position + 1),
          650, 650);

      h_FRONT_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->Draw("colz");

      //move the stat box
      gStyle->SetStatX(.9);
      gStyle->SetStatY(.9);

      c_Front_Field_XY_Dist_Dummy->Update();

      c_Front_Field_XY_Dist_Dummy->SaveAs(
          Form("%s/Front_Field_XY_Distribution_primary_%d_%d_to_%d.C",
               SAVE_PREFIX[primary_ID].c_str(), primary_ID, cut_position,
               cut_position + 1));
      c_Front_Field_XY_Dist_Dummy->SaveAs(
          Form("%s/Front_Field_XY_Distribution_primary_%d_%d_to_%d.png",
               SAVE_PREFIX[primary_ID].c_str(), primary_ID, cut_position,
               cut_position + 1));

      //electrons generated from -20 to 0
      TCanvas* c_Back_Field_XY_Dist_Dummy = new TCanvas(
          "c_Back_Field_XY_Dist_Dummy",
          Form("XY hit distribution for primary %d generated from %d to %d",
               primary_ID, cut_position * -1 - 1, cut_position * -1),
          650, 650);

      h_BACK_FIELD_XY_DIST_DUMMY[primary_ID][cut_position]->Draw("colz");

      //move the stat box
      gStyle->SetStatX(.9);
      gStyle->SetStatY(.9);

      c_Back_Field_XY_Dist_Dummy->Update();

      c_Back_Field_XY_Dist_Dummy->SaveAs(
          Form("%s/Back_Field_XY_Distribution_primary_%d_%d_to_%d.C",
               SAVE_PREFIX[primary_ID].c_str(), primary_ID,
               cut_position * -1 - 1, cut_position * -1));
      c_Back_Field_XY_Dist_Dummy->SaveAs(
          Form("%s/Back_Field_XY_Distribution_primary_%d_%d_to_%d.png",
               SAVE_PREFIX[primary_ID].c_str(), primary_ID,
               cut_position * -1 - 1, cut_position * -1));
    }
  }
